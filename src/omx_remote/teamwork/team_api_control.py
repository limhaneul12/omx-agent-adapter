...
from omx_remote.teamwork.team_api_command_runner import (
    _nonempty_string_items,
    _optional_bool,
    _optional_int,
    _optional_string,
    _optional_string_items,
    _run_team_api_command as _run_team_api_command_with_runner,
)
...
async def _run_team_api_command(
    ...
    """Run one Team API command through this module's patchable seam. Args: action [str] names the Team API action; payload [TeamApiControlPayload] is the typed msgspec payload. Returns: OmxCommandResult with the completed command result."""
    result = await _run_team_api_command_with_runner(
        action=action,
        payload=payload,
        command_runner=run_omx_command,
    )
    return result
