...
def test_codex_probe_suite_uses_fake_runner_without_shelling_out() -> None:
    ...
    def fake_runner(command: tuple[str, ...]) -> ProbeProcessOutput:
        ...
        if command == ("codex", "exec", "--help"):
            return ProbeProcessOutput(exit_code=0, stdout="Usage: codex exec --json\n", stderr="")
    ...
    assert result_by_capability["exec_json"].support_status == ProbeSupportStatus.SUPPORTED
    assert result_by_capability["plugin_help"].support_status == ProbeSupportStatus.UNSUPPORTED
...
def test_codex_probe_parses_json_stdout_when_available() -> None:
    ...
    result = run_codex_probe_suite(fake_runner)
    features_probe = next(probe for probe in result.results if probe.capability == "features_json")
