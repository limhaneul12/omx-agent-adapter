...
from teamwork.team_api_snapshot import (
...
@patch("teamwork.team_api_snapshot.run_omx_command")
def test_read_team_api_list_tasks_rejects_count_mismatch(mock_run_omx_command: Mock) -> None:
...
@patch("teamwork.team_api_snapshot.run_omx_command")
def test_read_team_api_read_events_rejects_count_mismatch(mock_run_omx_command: Mock) -> None:
    mock_run_omx_command.return_value = Mock(
        returncode=0,
    ...
    with pytest.raises(
        ...
        asyncio.run(read_team_api_read_events(TeamApiReadEventsRequest(team_name="alpha")))
