...
def test_read_github_pull_request_status_reports_open_pr_review_and_checks(
    ...
    def fake_read_github_api_json(repo_root: str, api_path: str):
        assert repo_root == str(tmp_path)
        payloads = {
            "/repos/limhaneul12/omx-agent-adapter/pulls?head=limhaneul12:feat/cockpit-pr-status-source&base=main&state=open": [
                {
                    "number": 10,
                    "state": "open",
...
def test_read_github_pull_request_status_reports_missing_open_pr(
    ...
    def fake_read_github_api_json(repo_root: str, api_path: str):
        assert api_path == "/repos/limhaneul12/omx-agent-adapter/pulls?head=limhaneul12:main&base=main&state=open"
        return []
